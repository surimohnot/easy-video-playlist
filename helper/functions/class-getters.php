...
class Getters {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $url Video or collection URL.
	 */
	...
	/**
	 * Get YouTube video items.
	 *
	 * @since 1.0.0
	 *
	 * @param string $url Video or collection URL.
	 * @param string $provider_type Provider type YouTube, Vimeo, url etc.
	...
	public static function get_youtube_video_items( $url, $provider_type, $api_key, $id, $max = 500 ) {
		...
		while ( $total < $max && $next_page_token ) {
	...
	/**
	 * Get YouTube video information using YouTube API.
	 *
	 * @since 1.0.0
	 *
	...
	public static function get_yt_video_information( $items, $provider_type, $api_key ) {
		...
		foreach ( $items as $item ) {
			...
			} elseif ( strpos( $provider_type, 'video' ) !== false ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $url URL of the video or collection.
	 */
	public static function get_oembed_providers( $url ) {

		// Check if the URL provider is youtube.
		$patterns = array(
			'/youtube\.com\/(?:watch\?v=|embed\/|v\/)([\w-]+)(?:$|&(?!list=))/i' => 'video',
			'/youtu\.be\/([\w-]+)/i'                   => 'video',
